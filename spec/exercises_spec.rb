...
describe Exercises do
	...
	describe "Exercise 9" do
		it "returns `true` if the given time is in a leap year" do
			leap_year = DateTime.new(2012)
      DateTime.stub(:now).and_return(leap_year)
      ...
      expect(result).to be_true
		...
		it "returns `false` if the given time is not a leap year" do
			not_leap_year = DateTime.new(2014)
      DateTime.stub(:now).and_return(not_leap_year)

      result = Exercises.ex9(not_leap_year)
