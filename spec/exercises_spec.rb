...
describe "Exercise 5" do
	it "iterates through an array and puts each element" do
		...
		# Must do the `:expect` statements then instantiate/run example case
...
describe "Extension Exercise: self.extremes" do
	it "returns hash with two keys correctly showing most and least values" do
	  result = Extensions.extremes(['x', 'x', 'y'])
	  expect(result).to eq({ :most => 'x', :least => 'y' })
	end

	it "returns array of tying strings along with most and least" do
