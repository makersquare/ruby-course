...
# # # # # # # # # # # # # # # # # # # # # #
  ...
  describe '#happy_hour?' do
    it "knows when it is happy hour (3:00pm to 4:00pm)" do
      # TODO: CONTROL TIME
      # shouldbe = Time.gm(2014, "jan",1,15,30,1)
      shouldbe = Time.parse('3 pm')
      allow(Time).to receive(:now).and_return(shouldbe)
      expect(@bar.happy_hour?).to eq(true)
    end

    it "is not happy hour otherwise" do
      # TODO: CONTROL TIME
      # shouldbe = Time.gm(2014, "jan",1,18,30,1)
      shouldbe = Time.parse('6 pm')
      allow(Time).to receive(:now).and_return(shouldbe)      
  ...
  context "During normal hours" do
    ...
    it "knows not to discount during normal hours" do
      ...
      shouldbe = Time.parse('6 pm')
      allow(Time).to receive(:now).and_return(shouldbe)   
      expect(@bar.get_price(5)).to eq("$5.00")
    end
  end
  ...
  context "During happy hours" do
    ...
    it "knows to discount during happy hour" do
      ...
      shouldbe = Time.parse('3 pm')
      allow(Time).to receive(:now).and_return(shouldbe)
      expect(@bar.get_price(5)).to eq("$2.50")
    end
  end
end
