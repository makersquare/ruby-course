...
# # # # # # # # # # # # # # # # # # # # # #
  ...
  describe '#happy_hour' do
    ...
    it "is not happy hour otherwise" do
      # TODO: CONTROL TIME
      Time.stub(:now).and_return(Time.parse("6 pm"))
  ...
  context "During normal hours" do
    ...
    it "bar must return a price for a given drink name" do
      Time.stub(:now).and_return(Time.parse("6 pm"))
      @bar.add_menu_item("whiskey", 4)
      expect(@bar.get_price("whiskey")).to eq(4)
    end
  end

  context "During happy hours" do
    # TODO: WRITE TESTS TO ENSURE BAR DISCOUNTS DURING HAPPY HOUR
    it "bar must return a price for a given drink name during happy hour" do
      Time.stub(:now).and_return(Time.parse("3 pm"))
      @bar.add_menu_item("whiskey", 4)
      expect(@bar.get_price("whiskey")).to eq(2)
    end
  end
end
