...
# # # # # # # # # # # # # # # # # # # # # #
  ...
  context "During normal hours" do
    ...
    it "stores information about transactions" do
      ...
      expect(@bar.transactions[0].item.name).to eq("Coccaine")
    ...
    it "rubs the lotion on its skin or else it gets the hose again" do
    end
    ...
    it "can return a list of sales in order of most popular items" do
      ...
      expect(@bar.most_popular_drinks[0][0]).to eq("Coccaine")
      expect(@bar.most_popular_drinks[0][1]).to eq(20)
      expect(@bar.most_popular_drinks.count).to eq(3)
    end

    it "can return a list of happy-hour sales and non-happy-hour sales" do
      # ring some stuff Tuesday non-happy-hour
      Time.stub(:now).and_return(TUESDAY_REGULAR_HOUR) # Tuesday non-Happy Hour
      4.times { @bar.ring_sale(@bar.menu_items[0]) }
      8.times { @bar.ring_sale(@bar.menu_items[2]) }
      ...
      expect(@bar.most_popular_happy_hour_drinks.max).to eq(["Coccaine", 12])
      expect(@bar.most_popular_non_happy_hour_drinks.max).to eq(["Earplugs", 8])

      expect(@bar.most_popular_happy_hour_drinks.count).to eq(2)
      expect(@bar.most_popular_non_happy_hour_drinks.count).to eq(2)
    end
