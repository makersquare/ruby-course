...
describe Bar do
	...
	describe '#happy_hour' do
		...
		it "is not happy hour otherwise" do
			# Stub Time.now to 4:30pm
			Time.stub(:now).and_return(Time.local(2014, "feb", 11, 16, 30))
	...
	describe '#get_price' do
		before do
			@bar.add_menu_item('Cosmo', 5.40)
			@bar.add_menu_item('Salty Dog', 7.80)
			@bar.add_menu_item('White Russian', 7.99)
		end
		...
		context "During normal hours" do
			...
			it "returns the price of a given drink" do
				expect(@bar.get_price('Cosmo')).to eql (5.40)
				expect(@bar.get_price('Salty Dog')).to eql (7.80)
				expect(@bar.get_price('White Russian')).to eql (7.99)
			end
		end
		...
		context "During happy hours" do
			...
			it "returns the discounted price of a drink" do
				expect(@bar.get_price('Cosmo')).to eql (5.40 *0.5)
				expect(@bar.get_price('Salty Dog')).to eql (7.80 *0.5)
				expect(@bar.get_price('White Russian')).to eql (7.99 *0.5)
			end
		end
	end
end
