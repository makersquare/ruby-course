...
# # # # # # # # # # # # # # # # # # # # # #
  ...
  describe '#happy_hour?' do
    it "knows when it is happy hour (3:00pm to 4:00pm)" do
      # TODO: CONTROL TIME
      Time.stub(:now).and_return(Time.new(2012,1,1,15))
      expect(@bar.happy_hour?).to eq(true)
    end

    it "is not happy hour otherwise" do
      # TODO: CONTROL TIME
      Time.stub(:now).and_return(Time.new(2012,1,1,16))
  ...
  it "gets non-discounted drink prices" do
      Time.stub(:now).and_return(Time.new(2012,1,1,16))
      @bar.add_menu_item("beer", 8.0)
      expect(@bar.get_price("beer")).to eq(8.0)
    end
  end
  ...
  context "During happy hours" do
    ...
    it "gets discounted drink prices" do
      ...
      @bar.happy_discount = 0.5
      @bar.add_menu_item("beer", 8.0)
      expect(@bar.get_price("beer")).to eq(4.0)
    end   
  end
end
