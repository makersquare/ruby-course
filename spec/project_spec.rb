...
describe 'Project' do
  ...
  it "task can be marked as complete by its id" do
    ...
    eating_better = TM::Task.new(1,2, "diet", 3)
    expect(eating_better.id).to eq 27 #create a task with an id of 2
    fitness_project.add_task(eating_better) #add this task to fitness project
    fitness_project.mark_task_complete(27) #mark task with id 2 complete
  ...
  it " can list complete_tasks" do
    ...
    expect(eating_better.id).to eq 28
    expect(sleep_8hours.id).to eq 29
    ...
    fitness_project.mark_task_complete(28) # taskid 2(eat) to complete=true
    fitness_project.mark_task_complete(29)
