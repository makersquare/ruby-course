...
describe 'Project' do
  ...
  xit "creates new task from a project" do
  project_one = TM::Project.new('one')
  ...
  xit "returns an array of completed tasks sorted by creation date" do
    TM::Task.id_counter = 1
    project_one = TM::Project.new('one')
  ...
  xit "returns an array of incomplete tasks sorted by priority" do
    TM::Task.id_counter = 1
    project_one = TM::Project.new('one')
    Time.stub(:now).and_return(Time.parse("3pm"))
    red = project_one.create_new_task("red", 5)
