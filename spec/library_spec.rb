...
describe Library do
  ...
  it "add new books and assigns it an id" do
    lib = Library.new
    lib.register_new_book("Nausea", "Jean-Paul Sartre", 2)
  ...
  it "allows a Borrower to check out a book by its id" do
    lib = Library.new
    lib.register_new_book("Green Eggs and Ham", "Dr. Seuss",1)
  ...
  it "knows who borrowed a book" do
    lib = Library.new
    lib.register_new_book("The Brothers Karamazov", "Fyodor Dostoesvky",1)
    book_id = lib.books.first.id
  ...
  it "does not allow a book to be checked out twice in a row" do
    lib = Library.new
    lib.register_new_book("Surely You're Joking Mr. Feynman", "Richard Feynman",1)
    book_id = lib.books.first.id
  ...
  it "allows a Borrower to check a book back in" do
    lib = Library.new
    lib.register_new_book("Finnegans Wake", "James Joyce",1)
    book_id = lib.books.first.id
  ...
  it "does not allow a Borrower to check out more than one Book at any given time" do
    ...
    lib = Library.new
    lib.register_new_book("Eloquent JavaScript", "Marijn Haverbeke",1)
    lib.register_new_book("Essential JavaScript Design Patterns", "Addy Osmani",1)
    lib.register_new_book("JavaScript: The Good Parts", "Douglas Crockford",1)
  ...
  it "returns available books" do
    ...
    expect(lib.available_books.first).is_a?(Book)
  ...
  it "after a book it returned, it can be checked out again" do
    lib = Library.new
    lib.register_new_book("Harry Potter", "J. K. Rowling", 1)
    book_id = lib.books.first.id
  ...
  it "returns borrowed books" do
    lib = Library.new
    lib.register_new_book("Eloquent JavaScript", "Marijn Haverbeke", 1)
    lib.register_new_book("Essential JavaScript Design Patterns", "Addy Osmani", 2)
    lib.register_new_book("JavaScript: The Good Parts", "Douglas Crockford", 3)

    # At first, no books are checked out
    expect(lib.borrowed_books.count).to eq(0)

    kors = Borrower.new("Michael Kors")
    book = lib.check_out_book(1, kors)

    # But now there should be one checked out book
    expect(lib.borrowed_books.count).to eq(1)
    expect(lib.borrowed_books.first).is_a?(Book)
  end
end
