...
describe Book do
  ...
  it "has a default status of available" do
    book = Book.new("The Prophet", "Kalil Gibrandi")
...
describe Library do
  ...
  it "starts with an empty array of books" do
    lib = Library.new("Travis County Library")
  ...
  it "check if members are added upon checkout" do
    ...
    expect(lib.members.first[:borrower]).to eq(jackson)
  end

  it "checks that a borrowed book is assigned to a borrower" do
    ...
    expect(lib.members_borrowed_books(jackson).first).to eq(book)
  end
  ...
  it "does not allow a Borrower to check out more than two Books at any given time" do
    ...
    book = lib.check_out_book(book_1.id, jackson)
    expect(book.title).to eq "Eloquent JavaScript"
    expect(lib.members_borrowed_books(jackson).length).to eq(1)

    book = lib.check_out_book(book_2.id, jackson)
    expect(book.title).to eq "Essential JavaScript Design Patterns"
    expect(lib.members_borrowed_books(jackson).length).to eq(2)
  ...
  it "after a book it returned, it can be checked out again" do
    ...
    #lib.check_in_book(book)
    expect( lib.check_in_book(book) ).to eq(book)
