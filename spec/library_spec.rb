...
describe Book do
  ...
  end #success all work perfectly

  it "has a default status of available" do
    book = Book.new("The Stranger", "Albert Camus")
    expect(book.status).to eq 'available'
  end #success all work perfectly had to add arguments to create a new book
  ...
  end #success all works perfectly had yo add arguments and had to create a new method check out for the book class
  ...
  it "can't be checked out twice in a row" do
    ...
    expect(book.status).to eq 'checked_out'
  end #success all works perfectly had to add arguments and had to create a new method check out for the book class
  ...
  it "can be checked in" do
    ...
    book.check_in
    expect(book.status).to eq 'available'
  end #success works perfectly had to take out an END from this error and create a new method for the book class called check in
end 
...
describe Library do
  ...
  it "starts with an empty array of books" do
    lib = Library.new("Marco's Library")
  ...
  end #success had to change this test a bit gave it names and used a BOOK NEW for the argument of register new book
  
  it "checks the first element of our book array" do
    lib = Library.new("Marco's Library")
    lib.register_new_book(Book.new("Nausea", "Jean-Paul Sartre", 1))
...
######HOMEWORK
  ...
  it "allows a Borrower to check out a book by its id" do
    lib = Library.new("Marco's Library")
    lib.register_new_book(Book.new("Green Eggs and Ham", "Dr. Seuss", 12))
  ...
  it "knows who borrowed a book" do
    lib = Library.new("Putins Library")
    lib.register_new_book(Book.new("The Brothers Karamazov", "Fyodor Dostoesvky", 001))
    book_id = lib.books.first.id
  ...
  it "does not allow a book to be checked out twice in a row" do
    lib = Library.new("Woodrow's Library")
    lib.register_new_book(Book.new("Surely You're Joking Mr. Feynman", "Richard Feynman", 002))
    book_id = lib.books.first.id
  ...
  it "does not allow a Borrower to check out more than one Book BUT LESS THAN TWO at any given time" do
    # yeah it's a stingy library
    lib = Library.new("Michael Jackson's Library")
    lib.register_new_book(Book.new("Eloquent JavaScript", "Marijn Haverbeke", 111))
    lib.register_new_book(Book.new("Essential JavaScript Design Patterns", "Addy Osmani", 006))
    lib.register_new_book(Book.new("JavaScript: The Good Parts", "Douglas Crockford", 222))
