...
let(:test_song2) {Songify::Song.new(title: "Goin' Out West",artist: "Tom Waits",album: "Bone Machine",year_published: 1992)}
  ...
  it "gets a song" do
    ...
    expect(result.first.artist).to eq("Elvis Costello")
    expect(result.first.title).to eq("Radio Radio")
    expect(result.first.year_published).to eq(1978)
    expect(result.first.album).to eq("This Year's Model")
    expect(result.first.rating).to eq(nil)
    expect(result.first.id).to eq(test_song1.id)
  end
