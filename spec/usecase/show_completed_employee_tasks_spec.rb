describe TM::ShowCompletedEmployeeTasks do
  it 'returns all complete tasks assigned to an employee' do
      ...
      completed_task1 = TM.db.mark_task_complete(created_task.id)
      completed_task2 = TM.db.mark_task_complete(created_task3.id)

      results = subject.run(eid: created_employee.id)

      expect(results.completed_tasks.last.description).to eq('the newest task')
