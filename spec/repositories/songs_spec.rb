...
describe Songify::Repos::Songs do
  ...
  let(:song) { Songify::Song.new('fake_title', 'fake_artist', 'fake_album', 'fake_genre_id')}
  ...
  describe '.get_song' do
    it 'returns the song object of the song id passed in' do
      ...
      expect(result.genre_id).to eq 'fake_genre_id'
    end
  end

  describe '.get_all_songs' do
    it 'returns an array of all song objects' do
      song2 = Songify::Song.new('fake_title2', 'fake_artist2', 'fake_album2', 'fake_genre_id2')
      songs.save_song(song)
