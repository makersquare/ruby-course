...
class Library
  ...
  def available_books
    avail = @books.select{|x| x.status[0] == "available"} #Refactor this into the books class.
  end

  def borrowed_books
    borr = @books.select{|x| x.status[0] == 'checked_out'} #Pretty much copyy available_books, but add ! . 
  end
